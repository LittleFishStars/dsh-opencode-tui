...
/** 计算若干段文本（各自独立折行）的总行数。 */
export function totalLines(parts, width) {
    let n = 0;
    for (const p of parts)
        n += estimateLines(p, width);
    return n;
}
/** 截断字符串到渲染宽度，加省略号。 */
...
function mapColor(color, bright) {
    if (color === undefined)
        return "default";
    if (bright) {
        switch (color) {
...
export function parseAnsi(text) {
    ...
    while ((match = sgrRegex.exec(text)) !== null) {
        if (match.index > cursor)
            parts.push({ text: text.slice(cursor, match.index), sgr: null });
        parts.push({ text: "", sgr: match[1] ?? "" });
        cursor = match.index + match[0].length;
    }
    if (cursor < text.length)
        parts.push({ text: text.slice(cursor), sgr: null });
...
function approximate256(code) {
    if (code < 8)
        return NAMED[code] ?? "default";
    if (code < 16)
        return mapColor(NAMED[code - 8], true);
    if (code < 232) {
        const cube = code - 16;
...
/** 会话标题兜底。 */
export function fallbackTitle(text, maxWords = 5, maxBytes = 40) {
