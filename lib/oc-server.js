    ...
    async handle(req, res) {
        ...
        if (sessionMatch) {
            ...
            if (method === "POST" && sub === "prompt") {
                ...
                void this.opts
                    .onPrompt(text, { resumeSessionId: state.dshSessionId, preset: PERMISSION_AGENTS[state.currentAgent], model: state.currentModel }, { onSession: (dshId) => this.bindDshSession(state.id, dshId) })
                    .then((dshId) => {
                    if (dshId)
