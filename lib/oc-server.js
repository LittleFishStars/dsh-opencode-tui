...
export class OcServer {
    ...
    runPrompt(state, text, opts) {
        ...
        void this.opts
            ...
            void this.opts
                .onPrompt(text, { resumeSessionId: undefined, preset: opts.preset, model: state.currentModel }, { onSession: (dshId) => this.bindDshSession(state.id, dshId) })
                .then((dshId) => {
                if (dshId)
                    this.bindDshSession(state.id, dshId);
            });
    ...
    async fallbackFilesystemScan(scope) {
        ...
        try {
            ...
            for (const state of this.store.sessions.values()) {
                ...
                if (candidates.some((c) => c.dshId === dshId))
                    continue;
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                for (const sd of sessionDirs) {
                    ...
                    // 缓存命中且 mtime 未变则跳过
                    if (this.titleCache.get(dshId) !== undefined)
                        continue;
