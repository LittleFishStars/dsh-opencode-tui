        ...
        if (sessionQuery) {
            try {
                ...
                if (records.length > 0) {
                    ...
                    let titles = new Map();
                    try {
                        const normalize = (id) => (id.startsWith("session-") ? id.slice(8) : id);
                    ...
                    const normalizeId = (id) => (id.startsWith("session-") ? id.slice(8) : id);
                    const out = [];
            ...
            for (const entry of entries) {
                ...
                for (const sd of sessionDirs) {
                    if (!sd.isDirectory())
                        continue;
                    const match = sd.name.match(/^session-(.+)$/) ?? sd.name.match(/^([0-9a-fA-F-]{36})$/);
                    if (!match || !match[1] || match[1] !== state.dshSessionId)
                        continue;
