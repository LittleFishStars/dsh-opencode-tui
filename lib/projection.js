...
export function applyEvent(messages, event) {
    switch (event.type) {
        ...
        case "assistant/chunk": {
            ...
            const closeBlock = (card, kind) => {
                const blocks = card.contentBlocks ?? [];
                const key = blockKey(chunk.index);
                const idx = blocks.findLastIndex((b) => b.kind === kind && b.key === key) ?? -1;
                if (idx === -1)
                    return card;
                const updated = [...blocks];
            ...
            if (last?.kind === "assistant" && !last.assembled) {
                ...
                if (chunk.type === "block-end") {
                    ...
                    if (chunk.text)
                        card = appendBlock(card, "text", chunk.text);
    ...
    for (const v of views) {
        if (v.kind === "user") {
            ...
            const blocks = v.contentBlocks && v.contentBlocks.length > 0
                ...
                : [...(v.text ? [{ kind: "text", text: v.text }] : []), ...(v.thinking ? [{ kind: "reasoning", text: v.thinking }] : [])].map((b) => ({ ...b, start: v.time, end: v.endedAt ?? v.time }));
