        ...
        switch (event.type) {
            ...
            case "tool/result": {
                ...
                const callID = first?.callId ??
                    ...
                    message.content?.find((c) => c.callId || c.toolCallId)?.callId ??
                    message.content?.find((c) => c.callId || c.toolCallId)?.toolCallId ??
                    "call_unknown";
            ...
            case "user/message": {
                ...
                if (data.source && data.source.kind !== "user")
                    break;
                const now = Date.now();
                // 去重：用户消息已由 POST /session/:id/message 添加（即时显示）。
                // 不能用时间窗口——上一轮未结束时发送的消息会被 DSH 排队，等上轮
                // 结束后才发 user/message 事件，此时早已超过任何短窗口，导致同一
                // 文本的 user 消息被添加两次（TUI 显示重复的问题框）。
                // 由于 POST 路径总是添加，事件仅需"最后一条 user 文本相同即跳过"。
            ...
            default: {
                if (event.type === "session/title") {
                    ...
                    if (data?.title) {
                        ...
                        if (state.dshSessionId)
                            this.opts.titleCache?.set(state.dshSessionId, data.title, Date.now());
                        this.store.pushSessionEvent(state, {
