...
function AssistantBody({ view, theme, width }) {
...
function ToolBody({ view, theme, width }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", width: width, children: [_jsxs(Box, { flexDirection: "row", children: [_jsxs(Text, { color: theme.textMuted, children: [nameText, ": "] }), tool.status === "running" ? (_jsx(Text, { color: theme.textMuted, wrap: "wrap", children: toolAction(tool.name) })) : (_jsx(Text, { color: theme.textMuted, wrap: "wrap", children: params || " " }))] }), tool.status !== "running" ? (_jsx(Box, { marginTop: 0, children: renderToolResult(tool, theme, width) })) : null] }));
}
/** 渲染单条消息（完整）。 */
export const MessageBlock = React.memo(function MessageBlock({ view, theme, width, }) {
    const borderColor = view.kind === "user" ? theme.secondary : view.kind === "assistant" ? theme.primary : theme.border;
    return (_jsx(Box, { flexDirection: "column", width: width + MSG_BORDER + MSG_PADDING, borderLeft: true, borderTop: false, borderBottom: false, borderRight: false, borderStyle: "bold", borderColor: borderColor, paddingLeft: MSG_PADDING, paddingRight: MSG_PADDING, children: view.kind === "user" ? (_jsx(UserBody, { view: view, theme: theme, width: width })) : view.kind === "assistant" ? (_jsx(AssistantBody, { view: view, theme: theme, width: width })) : (_jsx(ToolBody, { view: view, theme: theme, width: width })) }));
});
// ── 高度估算（虚拟滚动用） ────────────────────────────────────────────────
import { estimateLines } from "../util.js";
/** 估算一条消息渲染后的行数（含 1 行安全余量）。 */
export function estimateMessageHeight(view, width) {
    ...
    if (view.kind === "user") {
        ...
        if (view.thinking !== "" && !view.assembled)
            lines += 1;
        if (view.text !== "")
