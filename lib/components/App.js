...
export function TuiApp({ actions, brand, commands }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", width: width, height: height + 1, children: [_jsxs(Box, { flexDirection: "row", width: width, flexGrow: 1, children: [_jsxs(Box, { flexDirection: "column", width: mainWidth, flexGrow: 1, children: [_jsx(Messages, { messages: snapshot.messages, width: mainWidth, height: Math.max(5, height - editorHeight), theme: theme, busy: busy, task: snapshot.working.task, spinFrame: snapshot.working.spinFrame, showInitial: snapshot.currentSessionId === null && !snapshot.loadingSession, loading: snapshot.loadingSession, brand: brand }), _jsx(Box, { borderTop: true, borderBottom: false, borderLeft: false, borderRight: false, borderColor: theme.border, flexDirection: "column", height: editorHeight, children: _jsx(Editor, { theme: theme, width: mainWidth, canSend: !busy, disabled: dialogOpen || approvalActive, onSubmit: (text) => actions.send(text), onExternalEditor: (current, done) => actions.openExternalEditor(current, done), onAtComplete: () => {
                                        openFilePicker("", (path) => editorRef.current?.(` ${path}`));
                                    }, onRegisterEditor: (set) => {
                                        editorRef.current = set;
                                    }, onValueChange: (value) => {
                                        editorEmptyRef.current = value === "";
                                    }, onHelpRequest: () => store.toggleDialog("help"), onSendBlocked: () => store.notify("warn", "Agent is working, please wait...") }) })] }), snapshot.showSidebar ? (_jsx(Box, { width: sbWidth, borderLeft: true, borderTop: false, borderBottom: false, borderRight: false, borderColor: theme.borderDim, children: _jsx(Sidebar, { theme: theme, width: sbWidth - 1, height: Math.max(5, height - editorHeight), session: snapshot.sessions.find((s) => s.id === snapshot.currentSessionId) ?? null, model: snapshot.model, cwd: snapshot.cwd }) })) : null] }), _jsx(StatusBar, { theme: theme, width: width, notification: snapshot.notification, model: snapshot.model, sessionLabel: snapshot.currentTitle || null, busy: busy }), snapshot.dialogs.quit ? (_jsx(ConfirmDialog, { theme: theme, width: width, height: height, title: "Quit", message: "Are you sure you want to quit?", onConfirm: () => actions.quit(), onCancel: () => store.closeDialog("quit") })) : null, snapshot.dialogs.help ? (_jsx(HelpDialog, { theme: theme, width: width, height: height, sections: HELP_SECTIONS })) : null, snapshot.dialogs.sessions ? (_jsx(ListDialog, { theme: theme, width: width, height: height, title: "Sessions", items: sessionItems, emptyText: snapshot.sessionsLoaded ? "No sessions" : "Loading...", onConfirm: (item) => {
                    store.closeDialog("sessions");
                    actions.switchSession(item.id);
                }, onClose: () => store.closeDialog("sessions") })) : null, snapshot.dialogs.commands ? (_jsx(ListDialog, { theme: theme, width: width, height: height, title: "Commands", items: commands, filterable: true, onConfirm: (item) => {
                    store.closeDialog("commands");
                    actions.runCommand(item.id);
                }, onClose: () => store.closeDialog("commands") })) : null, snapshot.dialogs.models ? (_jsx(ListDialog, { theme: theme, width: width, height: height, title: "Models", items: modelItems, emptyText: "No model selection", onConfirm: () => store.closeDialog("models"), onClose: () => store.closeDialog("models") })) : null, snapshot.dialogs.theme ? (_jsx(ListDialog, { theme: theme, width: width, height: height, title: "Theme", items: themeItems, filterable: false, onConfirm: (item) => {
                    store.closeDialog("theme");
                    actions.setTheme(item.id);
                }, onClose: () => store.closeDialog("theme") })) : null, snapshot.dialogs.filepicker && filePicker ? (_jsx(FilePickerDialog, { theme: theme, width: width, height: height, state: filePicker, onChange: setFilePicker, onPick: (path) => {
