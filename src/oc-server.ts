...
export class OcServer {
  ...
  private touchSession(state: SessionState): void {
  ...
  private applyRequestModel(state: SessionState, payload: Record<string, unknown>): void {
  ...
  private runPrompt(state: SessionState, text: string, opts: { preset?: string }): void {
    void this.opts
      .onPrompt(
        text,
        {
  ...
  /** v2 端点（/api/*）。返回是否已处理。 */
  private async handleApi(path: string, method: string, body: string, url: URL, req: IncomingMessage, res: ServerResponse): Promise<boolean> {
    // ── v2 端点 ──
  ...
  private async handleLegacySession(path: string, method: string, body: string, url: URL, req: IncomingMessage, res: ServerResponse): Promise<boolean> {
    // 旧 /session/:id/* 子路由
  ...
  private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    // ── v2 端点 ──
    if (await this.handleApi(path, method, body, url, req, res)) return;
    // 旧 /session/:id/* 子路由
    if (await this.handleLegacySession(path, method, body, url, req, res)) return;
