...
/** 缺失时编译 fork；返回是否可用。 */
function ensureBuilt(binary: string, ctx: Context): boolean {
  if (existsSync(binary)) return true;
  const forkDir = dirname(binary);
  if (!existsSync(join(forkDir, "go.mod"))) {
    ctx.logger.error(
      `dsh-opencode-tui: opencode fork 不存在（${forkDir}）。请先克隆并编译：` +
...
function apply(ctx: Context, config: PluginConfig) {
  ...
  if (process.env.DSH_BRIDGE === "1") {
    ctx.logger.warn(
      "dsh-opencode-tui: DSH_BRIDGE 已设置但本插件仍被激活——bridge.yml 覆盖层未生效（--patch bridge.yml 缺失？），拒绝递归启动。",
    );
    return;
  }
