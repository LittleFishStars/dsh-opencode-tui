...
function parseResumeSession(args: readonly string[] | undefined): string | undefined {
...
function apply(ctx: Context, config: PluginConfig) {
  ...
  // ── 会话元信息（侧边栏/会话列表） ──────────────────────────────────────
  let messagesRef: MessageView[] = [];
  let currentSessionId: string | null = null;

  const updateSessionMeta = (
    sessionId: string,
    patch: Partial<SessionMeta> | ((prev: SessionMeta) => Partial<SessionMeta>),
  ): void => {
  ...
  const setCurrentMessages = (msgs: MessageView[]): void => {
    messagesRef = msgs;
  ...
  // ── 动作实现 ─────────────────────────────────────────────────────────────
  const externalEditor = (current: string, done: (text: string) => void): void => {
  ...
  const actions: TuiActions = {
    ...
    cancel: () => {
      if (manager.current) manager.cancel();
    },
    newSession: () => {
      void (async () => {
        try {
    ...
    runCommand: (commandId) => {
      switch (commandId) {
        ...
        case "resume": {
          ...
          if (target) actions.switchSession(target.id);
          else store.notify("warn", "No sessions available");
          break;
        }
        case "compact": {
          const owned = manager.current;
          ...
          if (owned && commands && typeof commands.execute === "function") {
            void (commands.execute as (agent: unknown, line: string, signal: AbortSignal) => Promise<unknown>)(
              owned.agent,
              "/compact",
  ...
  // ── 进程信号与卸载 ──────────────────────────────────────────────────────
  const onSignal = (): void => actions.quit();
