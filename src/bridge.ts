...
// ── 桥消息模型（对齐 opencode message.Message） ────────────────────────────
...
function mapReason(kind: string | undefined): string {
  switch (kind) {
    case "completed":
      return "end_turn";
    case "aborted":
...
function aggregate(views: MessageView[]): BridgeMessage[] {
  const out: BridgeMessage[] = [];
  let cur: BridgeMessage | null = null;
  for (const v of views) {
    if (v.kind === "user") {
      ...
      cur = {
        ...
        toolResults: [],
        finish:
          v.finished && v.endedAt !== undefined
...
function apply(ctx: Context, config: PluginConfig) {
  ...
  const respond = (id: number, result: unknown): void => send({ id, result });
  const fail = (id: number, message: string, code = "ERROR"): void =>
  ...
  const getSession = (sessionId: string): BridgeSession => {
  ...
  /** 重聚合并推送变化消息。节流调用方负责。 */
  const republish = (sessionId: string): void => {
    ...
    for (const m of next) {
      if (!oldIds.has(m.id)) emit("message/created", { sessionId, message: m });
    }
    s.messages = next;
  };
  ...
  const scheduleRepublish = (sessionId: string): void => {
  ...
  ctx.on("session/event", (session: Session, event: SessionEvent) => {
    ...
    if (event.type === "turn/start") {
      ...
      s.busy = false;
      const reason = mapReason((event.data as { reason?: { kind?: string } }).reason?.kind);
  ...
  const handlers: Record<string, (params: Record<string, unknown>) => Promise<unknown>> = {
    "session.create": async (params) => {
      ...
      s.title = (params.title as string | undefined) ?? "New Session";
      return {
        id: owned.sessionId,
        title: s.title,
    ...
    "agent.summarize": async (params) => {
      const id = String(params.sessionId);
      const commands = ctx.get("commands") as { execute?: unknown } | undefined;
      const owned = manager.current;
      if (owned && owned.sessionId === id && commands && typeof commands.execute === "function") {
        await (commands.execute as (agent: unknown, line: string, signal: AbortSignal) => Promise<unknown>)(
          owned.agent,
          "/compact",
    ...
    "approval.decide": async (params) => {
      const outcome = String(params.outcome) as "allowed-once" | "rejected";
      approvals.decide(outcome);
      return true;
    },
    "question.answer": async (params) => {
      const answer = params.answers as AskUserQuestionAnswer;
  ...
  rl.on("line", (line) => {
    ...
    void Promise.resolve(handler(req.params ?? {}))
      ...
      .catch((error: unknown) => fail(req.id!, error instanceof Error ? error.message : String(error)));
  });
