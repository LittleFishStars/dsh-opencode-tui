...
export async function handleLegacySession(
  ...
  if (legacySessionMatch) {
    ...
    if (method === "GET") await ctx.store.waitHydrate();
    ...
    if (method === "POST" && sub === "prompt_async") {
      ...
      if (text) {
        void ctx.sendPrompt(text, { resumeSessionId: state.dshSessionId }).then((dshId) => {
          if (dshId) ctx.bindDshSession(state.id, dshId);
        });
      }
 ...
 *  返回是否已处理。 */
export async function handleLegacyMisc(
  ctx: RouterContext,
  path: string,
  method: string,
  body: string,
  ...
  if (path === "/session" && method === "GET") {
    // 直查 DSH sessionQuery（权威数据源），而非仅读兼容层内存
    const scope = url.searchParams.get("scope");
    const list = await ctx.listSessions(scope);
  ...
  // 诊断端点：返回会话列表调试信息
  if (path === "/api/diag-sessions" && method === "GET") {
    ...
    const storeInfo = [...ctx.store.sessions.values()].map((s) => ({
      id: s.id?.slice(0, 12),
      dshId: s.dshSessionId?.slice(0, 12),
