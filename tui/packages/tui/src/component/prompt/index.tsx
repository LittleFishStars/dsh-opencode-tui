...
export function Prompt(props: PromptProps) {
  ...
  onCleanup(() => {
    ...
    // 只清自己注册的 inputTarget：路由切换时旧 Prompt 卸载不能误清新 Prompt
    // 的 target，否则输入层（Enter 提交等 keymap 绑定）会失效。
    setInputTarget((current) => (current === input ? undefined : current))
    props.ref?.(undefined)
  })
  createEffect(() => {
    if (!input || input.isDestroyed) return
    ...
    // DSH 兼容层：无条件 focus（focus() 幂等）。OpenTUI 的自动聚焦会把
    // input.focused 置 true 但不经过 renderer.focusRenderable，导致
    // currentFocusedEditor 跟踪缺失、keymap 的 input.submit（Enter）失效。
    input.focus()
  })
        ...
        >
          ...
          >
            <textarea
              ...
              minHeight={1}
              maxHeight={maxHeight()}
              // DSH 兼容层：渲染层直接绑定 Enter 提交。多行 textarea 默认
              // return=newline，提交走 keymap 的 input.submit（依赖 renderer
              // 的 currentFocusedEditor 跟踪，路由切换后可能失效）。
